...
class PerformanceCollector:
    ...
    def _cleanup_old_data(self):
        ...
        while (
            ...
            and performance_data["timestamps"][0] < cutoff_time
        ):
...
def render_performance_dashboard():
    ...
    if performance_data["timestamps"]:
        ...
        if performance_data["response_times"]:
            axes[1, 1].plot(
                performance_data["timestamps"][
                    : len(performance_data["response_times"])
                ],
    ...
    with col1:
        st.metric(
            "Cache Size", f"{cache_stats['cache_size']}/{cache_stats['max_cache_size']}"
        )
    ...
    with col2:
        if st.button("💾 Cleanup Memory"):
            ...
            st.success(
                f"Memory cleanup completed! Freed {cleanup_stats['memory_freed_mb']:.1f} MB"
            )
            st.rerun()
...
def get_performance_alerts() -> List[str]:
    ...
    if memory_stats["process_memory_percent"] > 80:
        alerts.append(
            f"⚠️ High memory usage: {memory_stats['process_memory_percent']:.1f}%"
        )

    if memory_stats["system_memory_percent"] > 90:
        alerts.append(
            f"🚨 Critical system memory: {memory_stats['system_memory_percent']:.1f}%"
        )
    ...
    if cache_stats["cache_size"] > cache_stats["max_cache_size"] * 0.9:
        alerts.append(
            f"⚠️ Cache nearly full: {cache_stats['cache_size']}/{cache_stats['max_cache_size']}"
        )
    ...
    if alerts:
        ...
        if any("memory" in alert.lower() for alert in alerts):
            if st.button("🧹 Auto Cleanup"):
                cleanup_stats = memory_manager.cleanup_memory()
                st.success(
                    f"Auto cleanup completed! Freed {cleanup_stats['memory_freed_mb']:.1f} MB"
                )
                st.rerun()
