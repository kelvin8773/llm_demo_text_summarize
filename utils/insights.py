...
CUSTOM_STOPWORDS = list(
    ENGLISH_STOP_WORDS.union(
        {"business", "use", "using", "user", "real", "area", "data", "information", 
         "system", "service", "application", "software", "technology", "process",
         "method", "approach", "solution", "implementation", "development"}
    )
)
...
def _initialize_spacy() -> None:
    ...
    if _nlp is None:
        try:
            ...
            logger.error("Please install the English model: python -m spacy download en_core_web_sm")
...
def _extract_noun_chunks(text: str) -> Set[str]:
    ...
    try:
        ...
        for chunk in doc.noun_chunks:
            ...
            if (phrase not in CUSTOM_STOPWORDS and 
                len(phrase) > MIN_PHRASE_LENGTH and
                phrase.replace(" ", "").isalpha()):  # Only alphabetic phrases
...
def _validate_input(text: str, top_n: int) -> None:
    ...
    if len(text.strip()) < 50:
        raise ValueError("Input text is too short for meaningful keyword extraction (minimum 50 characters)")
...
def extract_keywords_phrases(text: str, top_n: int = DEFAULT_TOP_N) -> List[str]:
    ...
    try:
        ...
        if not candidates:
            logger.warning("No noun chunks found, falling back to basic keyword extraction")
